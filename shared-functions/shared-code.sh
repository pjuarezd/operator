...
# usage: wait_resource_status <namespace> <resourcetype> <resourcename> <json path style field> [<timeout>]
function wait_resource_field_to_exist() {
  # This function will wait until the resource has the requested json path field
  # wait_resource_status ns-1 Tenant my-tenant .status
  # wait_resource_status ns-1 Tenant my-tenant .status 1200

  local namespace=$1
  local resourcetype=$2
  local resourcename=$3
  local fieldselector=$4
  ...
  while true; do
    if kubectl get "$resourcetype" -n "$namespace" "$resourcename" -ojson | jq -e $fieldselector >/dev/null 2>&1; then
      ...
      echo "$resourcetype/$resourcename field '$fieldselector' not found, waiting for $INTERVAL seconds."
      sleep $INTERVAL
    fi
    ...
    if [ "$ELAPSED_TIME" -ge "$TIMEOUT" ]; then
      echo "Timeout waiting for $resourcetype/$resourcename field '$fieldselector' to be created."
      exit 1
    fi
  done
}